...
class Navigation {
  ...
  attachEvents(callback) {
    ...
    this.nav.addEventListener("pointerenter", () => {
      this.nav.addEventListener("pointermove", this._pointerMoveHandler, {
        passive: true,
      })
    })
  ...
  updateVisibleItems() {
    ...
    for (let row = a; row <= o; row++) {
      for (let col = s; col <= n; col++) {
        ...
        let candidate =
          Math.abs((row * this.columns + col) % this.itemCount) %
        ...
        while (attempts < this.imageUrls.length) {
          ...
          for (let placed of placedCandidates) {
            ...
            if (
              Math.sqrt(dCol * dCol + dRow * dRow) <= minSpacing &&
  ...
  init() {
    if (this.container && this.canvas) {
      ...
      this.currentX = this.targetX =
        (this.container.clientWidth - totalWidth) / 2
      this.currentY = this.targetY =
        (this.container.clientHeight - totalHeight) / 2
      this.updateVisibleItems()
...
const projects = [
  {
    ...
    title: "Phenom",
    year: "UX Strategist",
    image: "https://i.ibb.co/WWCxWtK8/603d61b96ca6f0c336165b6e-DSC09310.webp",
  },
  {
    id: 2,
    title: "Independent",
    year: "UX Consultant",
    image: "https://i.ibb.co/TDk7M7zh/1-KE8g-FA6w4wud1u-DAr-A4-Yv-Q.webp",
  },
  {
    id: 3,
    title: "Publicis Sapient",
    year: "Sr. Product Designer",
    image: "https://i.ibb.co/mCK7D4Xb/1-Oen-HCQm-Ga10wp-Cz-R8o9-Pt-A.webp",
  },
  {
    id: 4,
    title: "Code & Theory",
    year: "ACD, Interactive",
    image: "https://i.ibb.co/GQM6jmpJ/1-1gjr-Eff-Bio-JYo-EAv-MWHf-KQ.webp",
  },
  {
    id: 5,
    title: "Elva Design Group",
    year: "Sr. UX Designer",
    image: "https://i.ibb.co/pvJvGhnt/ZAX02330-1.webp",
  },
  {
    id: 6,
    title: "Huge",
  ...
  },
  {
    ...
    title: "LiveArea",
    year: "Manager, UX",
    image: "https://i.ibb.co/Xr97WzXX/DSC06055-1.webp",
  },
  {
    id: 8,
    title: "PHI → NYC",
    year: "Paradigm Shift",
    image: "https://i.ibb.co/fYM2drZG/1-UATKEet6-Vtc-Lk-A8r-Yspj-FQ.webp",
  },
  {
    id: 9,
    title: "Workarea",
    year: "UX Designer",
    image: "https://i.ibb.co/N2yd0jJV/1-l-EJetj-Ut85-CTbb24d-BTFk-A.webp",
  },
  {
    id: 10,
    title: "O3 World",
    year: "UX Designer",
    image: "https://i.ibb.co/LXmyD2hq/IMG-20170104-184947-650.webp",
  },
  {
    id: 11,
    title: "160 Over 90",
    year: "Interactive Designer",
    image: "https://i.ibb.co/CpkzWmRn/IMG-20151206-143245434-01.webp",
  },
]
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  function initLifeScroll() {
    ...
    if (
      lifePage &&
      lifePage.classList.contains("active") &&
      typeof initLifeScrollProgress === "function"
    ) {
...
;(function () {
  ...
  scrollContainer.addEventListener("scroll", debounce(toggleVisibility, 50), {
    passive: true,
  })
...
// WebGL background: flawless 1:1, always covers, never distorts, pointer always true.
// The canvas is always at least as large as the viewport diagonal (cover), so the object area always fills the viewport with no stretching.
// Only two frustum/camera scales: desktop/mobile.
...
const fragment = `
  ...
  uniform vec4 resolution; // x=canvasW, y=canvasH, z=aspectW, w=aspectH
  uniform vec2 mouse;
...
const DESKTOP_FRUSTUM = 0.75;
const MOBILE_FRUSTUM = 0.875;
...
class Sketch {
  constructor({ dom }) {
    ...
    this.camera = new THREE.OrthographicCamera(
      -DESKTOP_FRUSTUM, DESKTOP_FRUSTUM,
      DESKTOP_FRUSTUM, -DESKTOP_FRUSTUM,
      -1e3, 1e3
    );
  ...
  resize() {
    ...
    // Calculate the diagonal to ensure canvas covers the viewport at any aspect
    const diag = Math.ceil(Math.sqrt(w * w + h * h));
    // Slightly increase for overscan/buffering
    const canvasSize = diag * 1.01;

    // Camera frustum: only two options
    const frustum = this.isMobile ? MOBILE_FRUSTUM : DESKTOP_FRUSTUM;
    ...
    // The renderer draws a square canvas, always larger than the viewport, and centers it
    this.renderer.setPixelRatio(dpr);
    this.renderer.setSize(canvasSize, canvasSize, false);
    this.renderer.domElement.width = canvasSize * dpr;
    this.renderer.domElement.height = canvasSize * dpr;
    this.renderer.domElement.style.width = `${canvasSize}px`;
    this.renderer.domElement.style.height = `${canvasSize}px`;

    // Pass canvas size (which is always square and covers the viewport) to the shader
    this.material.uniforms.resolution.value.x = canvasSize * dpr;
    this.material.uniforms.resolution.value.y = canvasSize * dpr;
    this.material.uniforms.resolution.value.z = 1;
    this.material.uniforms.resolution.value.w = 1;

    // For pointer normalization
  ...
  mouseEvents() {
    ...
    const getNormXY = (clientX, clientY) => {
      // Map pointer to the square canvas coordinates: -0.5...0.5 in both axes
      // Find offset of the canvas relative to the viewport (canvas is centered and larger)
      const w = this.viewportW, h = this.viewportH, s = this.canvasSize;
      const offsetX = (s - w) / 2;
      const offsetY = (s - h) / 2;
      // Canvas is centered, so pointer in viewport maps to canvas as:
      let x = ((clientX + offsetX) / s) - 0.5;
      let y = 0.5 - ((clientY + offsetY) / s);
    ...
    document.addEventListener("mousemove", (e) => {
      [this.mouse.x, this.mouse.y] = getNormXY(e.clientX, e.clientY);
    });
    document.addEventListener("touchmove", (e) => {
      if (!e.touches[0]) return;
      [this.mouse.x, this.mouse.y] = getNormXY(e.touches[0].clientX, e.touches[0].clientY);
    }, { passive: false });
  }
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  if (container) {
    ...
    window.addEventListener("scroll", debounce(toggleGlassy, 50), {
      passive: true,
    })
    toggleGlassy()
  }
...
;(function () {
  ...
  if (scrollElem === window) {
    ...
    content.addEventListener("scroll", debounce(checkScroll, 50), {
      passive: true,
    })
  }
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  async function runMastSequence() {
    ...
    const glassyButtonCont = document.querySelector(
      ".connect-container .glassy-button-container",
    )
    if (glassyButtonCont) {
    ...
    mast.ps.forEach((p) => {
      if (p) p.classList.remove("mast-fadein")
    })
  ...
  setInterval(() => {
    const isConnect = document
      .getElementById("page-connect")
...
function showInstructionWithDelay() {
  ...
  if (
    ...
    !instructionText ||
    !activePage ||
    !instructionTexts[activePage.id]
  ) {
