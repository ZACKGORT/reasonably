...
document.addEventListener('DOMContentLoaded', () => {
  ...
  cardsContainer.style.transformOrigin = '50% 50%';

  let isDragging = false;
  let startAngle = 0;
  let initialRotation = 0;

  // Helper function: calculates the angle in radians between (x, y) and the center (cx, cy)
  ...
  const onPointerDown = (e) => {
    ...
    // Retrieve the current rotation from the computed style
    const computedStyle = window.getComputedStyle(cardsContainer);
    const transform = computedStyle.transform;
    if (transform && transform !== 'none') {
      // In our design, the transform may be "translate(-50%, -50%)" along with rotation.
      // We'll extract the rotation from the matrix.
      const values = transform.match(/matrix\(([^)]+)\)/);
      if (values) {
        const parts = values[1].split(',').map(parseFloat);
        // The rotation can be calculated with Math.atan2(b, a)
        initialRotation = Math.atan2(parts[1], parts[0]);
      }
    } else {
      initialRotation = 0;
    }
  ...
  const onPointerMove = (e) => {
    ...
    // Combine the existing "translate" with the new rotation.
    cardsContainer.style.transform = `translate(-50%, -50%) rotate(${newRotation}rad)`;
  };
  ...
  cardsContainer.addEventListener('pointerdown', onPointerDown);
  cardsContainer.addEventListener('pointermove', onPointerMove);
  cardsContainer.addEventListener('pointerup', onPointerUp);
  cardsContainer.addEventListener('pointercancel', onPointerUp);
